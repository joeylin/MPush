...
exports.index = function (req, res, next) {
  ...
  Topic.getFullTopic(topic_id, ep.done(function (message, topic, author, replies, like_count) {
    if (message) {
      ep.unbind();
...
exports.create = function (req, res, next) {
  res.render('topic/edit', {isPublic: true});
};
...
exports.pagePut = function (req, res, next) {
  ...
  var edit_error =
      content === '' ?
    '内容不能是空的。' :
    (content.length <= 10 ? '内容字数太少。' : '');

  if (edit_error || !req.body.topic_tags) {
    return res.send({
      code: 404,
      info: edit_error || '没写标签'
    });
  }
  var title;
  if (req.body.title) {
  ...
  Topic.newAndSave(title, req.body.topic_tags, isPublic, content, req.session.user._id, function (err, topic) {
    if (err) {
      return next(err);
    }
    ...
    if (String(topic.author_id) === req.session.user._id || req.session.user.is_admin) {
      ...
      if (title === '') {
        Tag.getAllTags(function (err, all_tags) {
          ...
          res.render('topic/edit', {action: 'edit', edit_error: '标题不能是空的。', topic_id: topic._id, content: content, tags: topic.tags, isPublic: topic.isPublic});
        });
      } else {
        //保存话题
...
exports.ajax_update = function (req, res, next) {
  if (!req.session.user) {
    res.redirect('/');
    return;
  }
...
exports.like = function (req, res, next) {
  ...
  Like.getLike(user._id, topic_id, function(err, like) {
    if (err) {
      return res.send({
        code: 404
      });
    ...
    Like.newAndSave(user._id, topic_id, function(err) {
      Topic.getTopic(topic_id, function(err, topic) {
        topic.like_count += 1;
        topic.save(function() {
          res.send({
...
exports.un_like = function(req, res, next) {
  var topic_id = req.body.topic_id;
  var user = req.session.user;
  Like.getLike(user._id, topic_id, function(err, like) {
    if (err) {
    ...
    Like.remove(user._id, topic_id, function(err) {
      Topic.getTopic(topic_id, function(err, topic) {
        topic.like_count -= 1;
...
exports.forward = function(req, res, next) {
  var topic_id = req.body.topic_id;
  var user = req.session.user;
  Topic.forward(user._id, topic_id, function(err, topic) {
    res.send({
      code: 200,
      id: topic._id
    });
