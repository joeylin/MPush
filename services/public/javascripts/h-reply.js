		...
		$.get(url, params, function(data) {
			if (data.code === 200) {
				...
				if (data.replies.length === 0) {
					str = '<div class="text-center">还没有人进行讨论。</div>';
				} else {
		...
		$box.on('click.inlineReply', '.h-inline-reply-btn', function() {
			...
			if ($(this).data('loaded')) {
				...
				if ($(this).data('opened')) {
					...
					$container.find('textarea').val('@' + user + ' ').focus();
					$(this).data('opened', true);
				}
				return;
			}
			...
			var topic_id = $media.closest('.media').data('id');
			$inline.css({display:'block'}).appendTo($mediaBody);
			$inline.find('textarea').val('@' + user + ' ').focus();
